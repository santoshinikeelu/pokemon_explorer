...
import { formatPokemonName, getPokemonByName, getPokemonSpecies, getTypeColor } from "@/lib/pokemon";
...
export async function generateStaticParams() {
  const pokemonList = await fetch("https://pokeapi.co/api/v2/pokemon?limit=151").then(res => res.json());
  
  return pokemonList.results.map((pokemon: { name: string }) => ({ name: pokemon.name }));
}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-1">
          <div className="flex flex-col items-center">
            <div
              ...
              style={{ backgroundColor: `${getTypeColor(pokemon.types[0].type.name)}20` }}
            >
              {imageUrl && (
                <Image
                  src={imageUrl}
          ...
          <div>
            ...
            <div className="space-y-3">
              {pokemon.stats.map(stat => (
                <div key={stat.stat.name} className="flex items-center space-x-2">
                  <span className="text-sm font-medium w-24">{stat.stat.name.toUpperCase()}</span>
            ...
            <div className="space-y-4">
              {pokemon.abilities.map(({ ability }) => (
                <div key={ability.name} className="p-4 border rounded-lg bg-white/80 dark:bg-gray-500">
                  <h3 className="font-semibold">{formatPokemonName(ability.name)}</h3>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {pokemon.moves.slice(0, 10).map(move => (
                <div key={move.move.name} className="p-2 border rounded-md text-sm">
